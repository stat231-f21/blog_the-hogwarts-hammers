...
# Read in geographic data for leaflet
leafletData <- readOGR(
  dsn = paste0(getwd(), "/data/LeafletData"),
  layer = "TM_WORLD_BORDERS_SIMPL-0.3",
  verbose = FALSE
)
# Read in the data combining suicide rates and hospital beds
suicideData <- read.csv("fullSuicideData.csv")
# Create data set with only 2016 data
suicideData2016 <- suicideData %>%
  filter(Year == "2016")
# Create data set with leaflet data and 2016 suicide data
leafletSuicideData2016 <- merge(leafletData, suicideData2016, by = "ISO3")
# Read in the news word distributions for the terms "mental", "health", and "needs"
usMentalHealthNeeds <- read.csv("data/USMentalHealthNeedsWords.csv")
# Create data set with suicide rate data in the US after 2000
usSuicideRatesPost2000 <- suicideRate %>%
  filter(Code == "USA", Year >= 2000)
# Read in data on suicides and guns by state in the US
SuicideStateWithInfo <- read_csv("SuicideStateWithInfo.csv", show_col_types = FALSE) 
```
...
```{r usSuicideRates}
# Create a line plot of suicide rates in the US after 2000
ggplot(usSuicideRatesPost2000, aes(x = Year, y = SuicideDeathRate)) +
  labs(y = "Suicide death rate per 100,000 people", 
...
```{r 2016Leaflet, echo=FALSE}
# Create bins with ranges of suicide rates
...
myPalette <- colorBin(
  ...
  bins = mybins)
# Defining the text that will appear when the mouse hovers over a country
hoverText <-
  paste("Country: ", leafletSuicideData2016@data$NAME, "<br/>",
        "Suicide Rate (2016): ", leafletSuicideData2016$SuicideDeathRate, "<br/>",
        sep = "") %>%
  lapply(htmltools::HTML)
# Creating world map of suicide rate per 100,000 in 2016
leaflet(leafletSuicideData2016) %>%
  addTiles() %>%
  # Set the view to fit the whole world
  setView(lat = 30, lng = 0, zoom = 1.5) %>%
  # Add the legend of colors and bins defined above
  addLegend(pal = myPalette, 
            values = leafletSuicideData2016$SuicideDeathRate, 
            opacity = .7, 
            title = "Suicide Death Rate<br/>Per 100k", 
            position = "bottomleft") %>%
  # Add circles whose color and size varries by suicide rate
  addCircles(lat = leafletSuicideData2016$LAT, lng = leafletSuicideData2016$LON,
             # Set size of circle
             radius = leafletSuicideData2016$SuicideDeathRate * 10000,
             # Set the opacity of circle and rim
             opacity = 1,
             fillOpacity = .7,
             weight = 1,
             group = "circles",
...
<iframe height="850" width="100%" frameborder="no" src="https://vkuntz23.shinyapps.io/WorldSuicideRates/"> </iframe>
...
```{r temperaturePlot, echo=FALSE, message=FALSE, warning=FALSE}
# Create a scatter plot of temperature vs suicide death rate
ggplot(suicideData2016, aes(x = SuicideDeathRate, y = TEMP)) +
  # Set x axis
  xlim(0, 33.5) +
  # Add title and axis titles
  labs(title = "How temperature impacts suicide rate",
...
# Set the seed so I get the same word cloud every time
set.seed(3000)
# Create word cloud with the words "mental", "health", and "needs" in the title
wordcloud(words = usMentalHealthNeeds$word, 
          freq = usMentalHealthNeeds$freq, 
          min.freq = 1, 
          max.words = 100, 
          rot.per = 0.35, 
...
```{r bedsSuicide, warning=FALSE, message=FALSE, echo=FALSE}
# Create a scatter plot of total number of beds vs suicide rate
ggplot(suicideData2016, aes(x = SuicideDeathRate, y = TOTAL_BEDS)) +
  # Set x axis
  xlim(0, 30) +
  # Add title and axis titles
  labs(title = "How number of mental health beds relates to suicide rate", 
       y = "Total number of mental health beds per 100,000 people", 
...
For this deep dive into the United States, there were predominantly four data sources:
  ...
  1. [Suicide Mortality Data](https://www.cdc.gov/nchs/pressroom/sosmap/suicide-mortality/suicide.htm) - Downloaded csv from the CDC. This dataset contained suicide rates and deaths by suicide for each state.
  2. [Gun Ownership Data](https://worldpopulationreview.com/state-rankings/gun-ownership-by-state) - Downloaded csv from the World Population review. This dataset contained data on the gun ownership percent in each state as well as the total guns in each state.
...
To create the plot shown in this section, I utilized the state dataset, map data, as well as Suicide Mortality Data. the first things I had to do was to join the state info with the data in the suicide mortality data so that I could use that new dataset for the ultimate plotting of the map. After some wrangling, I was able to do this and then combine it with the map data and setting that dataset as a 'simple feature' object which is the standard data container for spatial data. after getting this dataset ready, it was just a matter of using ggplot and the aes functions to create a US Map that had each state colored based on the suicide rate in that state. I had also created an identical plot for total deaths by suicide but after looking at the output realized that plot did not show much other than the population density as states with higher populations naturally had higher suicide deaths than those with small populations. Lastly, I wanted to add some interactivity to the plot so that there was a hover feature that showed the exact rate in each state. To do this, I used the *plotly* package which allowed me to plot the map with *dynamicTicks* set to True which behaved the way I intended.
...
# plotting the plot object dynamically with n o tick marks
...
y <- ggplot(graphs_map, aes(fill = gunOwnership, text = paste("State: ", id))) +
  geom_sf(size = .2, colour = "white") +
  scale_fill_viridis(option = "magma", direction = -1) +
...
# plotting map without longitude/lattitude ticks
...
ggplot(SuicideStateWithInfo, aes(x = gunOwnership, y = RATE)) +
  # adding in points
  geom_point() +
