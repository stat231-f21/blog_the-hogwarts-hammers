...
#VIETTA
```{r wrangling}
# make the year a column as opposed to a column header
suicideRateByYear <- suicideRate %>% 
  pivot_longer(cols = starts_with("20"), names_to = "Year", values_to = "RateWithRange") %>%
  # splitting the rate and the range into separate columns
  mutate(Rate = as.numeric(gsub(" .*$","",RateWithRange)), Range = gsub("^.* ", "", RateWithRange))
# calculating gender ratio of suicide rates
maleSuicideRate <- suicideRateByYear %>%
  filter(suicideRateByYear$Sex == 'Male') %>%
  select(Country, Year, Rate) %>%
  rename(MaleRate = Rate)

femaleSuicideRate <- suicideRateByYear %>%
  filter(suicideRateByYear$Sex == 'Female') %>%
  select(Country, Year, Rate) %>%
  rename(FemaleRate = Rate)

suicideRateCompare <- maleSuicideRate %>%
  left_join(femaleSuicideRate) %>%
  mutate(Ratio = MaleRate / FemaleRate)
```

```{r worldMap}
world <- ne_countries(scale = "medium", returnclass = "sf")
suicideRate2017 <- suicideRateCompare %>%
  filter(suicideRateCompare$Year == '2017') %>%
  select(Country, Ratio) %>%
  rename(name = Country)

worldRates <- world%>%
  left_join(suicideRate2017)

p <- ggplot(data = worldRates, aes(fill = Ratio, text = paste("Country: ", name))) +
  geom_sf(size=.2, colour="white") +
#  scale_color_gradient2(midpoint=1, low="blue", mid="grey", high="red")
scale_fill_viridis(option = "magma", 
                   breaks=c(1, 3, 5, 7, 9, 11), 
                   name="Ratio of male to female suicide rate", 
...
```{r}
missingRates <- worldRates %>% filter(is.na(worldRates$Ratio)) %>% select(name)
```

```{r worldMap}
world <- ne_countries(scale = "medium", returnclass = "sf")
suicideRatebyCountry <- suicideRateByYear %>%
  filter(suicideRateByYear$Year == '2017') %>%
  select(Country, Rate) %>%
  rename(name = Country)

worldSuicideRates <- world%>%
  left_join(suicideRatebyCountry)

p <- ggplot(data = worldSuicideRates, aes(fill = Rate, text = paste("Country: ", name))) +
  geom_sf(size=.2, colour="white") +
#  scale_color_gradient2(midpoint=1, low="blue", mid="grey", high="red")
